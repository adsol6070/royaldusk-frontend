...
const FilterSidebarComponent = ({
  ...
  const convertedPrices = data.map((item) =>
    convertPrice(parseFloat(item.price), baseCurrency, selectedCurrency)
  );

  const maxPrice =
    convertedPrices.length > 0 ? Math.ceil(Math.max(...convertedPrices)) : 2000;

  // Safely extract locations
  const locations = Array.from(
    new Set(data.map((item) => getLocationForFilter(item)).filter(Boolean))
  );

  const tags = Array.from(new Set(data.map((item) => item.tag))).filter(
    Boolean
  );
  const availabilityOptions = Array.from(
    new Set(data.map((item) => item.tourAvailability))
  ).filter(Boolean);
...
const TourAndActivitiesPage = () => {
  ...
  const handleFilterChange = useCallback(
    ({ priceRange, locations, tags, availability, searchTerm, currency }) => {
      let filtered = [...tours];
      ...
      filtered = filtered.filter((tour) => {
        ...
        return price >= minPriceInBase && price <= maxPriceInBase;
      });
      ...
      // Tag filter
      if (tags.length) {
        filtered = filtered.filter((tour) => tags.includes(tour.tag));
      }
    ...
    [tours, convertPrice, baseCurrency]
  );
  ...
  const handleSort = (value) => {
    ...
    const sorted = [...filteredTours].sort((a, b) => {
      switch (value) {
        case "price-low":
          return (
            convertPrice(parseFloat(a.price), baseCurrency, selectedCurrency) -
            convertPrice(parseFloat(b.price), baseCurrency, selectedCurrency)
          );
        case "price-high":
          return (
            convertPrice(parseFloat(b.price), baseCurrency, selectedCurrency) -
  ...
  const totalDestinations = Array.from(
    new Set(tours.map((tour) => getLocationForFilter(tour)).filter(Boolean))
  ).length;
                    ...
                    <TourCard key={tour.id}>
                      <TourImage>
                        ...
                        {tour.tourAvailability && (
                          <div
                            className={`status-badge ${tour.tourAvailability.toLowerCase()}`}
                          >
                            {tour.tourAvailability}
                          </div>
                        )}
                        ...
                        {tour.tag && (
                          <div className="tag-badge">{tour.tag}</div>
                        )}
